...
const BASE_URL = __ENV.BASE_URL || "http://localhost:3001/dashboard/peraturan";

export default function () {
  // Test 1: List assets without query
  const res1 = http.get(`${BASE_URL}/api/rpc/regulation?pageSize=20`, {
    headers: {
  ...
  const res2 = http.get(
    `${BASE_URL}/api/rpc/regulation?pageSize=10&query=test`,
    {
      headers: {
        "Content-Type": "application/json",
      },
  ...
  check(res2, {
    "regulation search status 200": (r) => r.status === 200,
    "regulation search response valid": (r) => {
      try {
        JSON.parse(r.body);
        return true;
      } catch (e) {
  ...
  const res3 = http.get(`${BASE_URL}/api/rpc/regulation?pageSize=15`, {
    headers: {
      "Content-Type": "application/json",
    },
  });
  ...
  check(res3, {
    "regulation pagination status 200": (r) => r.status === 200,
    "regulation has nextCursor field": (r) => {
      try {
        const body = JSON.parse(r.body);
...
export function handleSummary(data) {
  return {
    "stressTest/result/regulation-stress-test-summary.json":
      JSON.stringify(data),
